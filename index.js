...
return  callback(numberList.reduce((total, num) => {
  return total += num;},0))
}
...
function processContains(item, list, callback) {
  /* CODE HERE */
  let filList = list.filter(val => val === item);
  let blean = (filList[0] === item);
  return callback(blean);
}
...
function getFullNames(runners) {
  /* CODE HERE */
  const fullNames = [];
  runners.forEach(runners => fullNames.push(`${runners.last_name}, ${runners.first_name}`));
  return fullNames;
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  let size = runners.filter(value => value.shirt_size === tShirtSize);
  return size;
...
function tallyUpDonations(runners) {
  const donate = runners.reduce((total, runners) => {return total + runners.donation;},0);
  return donate;
}
...
const counter2 = counterMakerWithLimit();
